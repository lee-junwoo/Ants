...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        self.chewing = 0
    def action(self, gamestate):
        if self.chewing == False:
            bee_selected = bee_selector(self.place.bees)
            if bee_selected:
                self.eat_bee(bee_selected)
                self.chewing = self.chew_duration
        else:
            self.chewing = self.chewing - 1
# END Problem 7
...
# END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and self.imposter == False:
            return bees_win()
        elif self.health <= 0 and self.imposter == True:
            self.place.remove_insect(self)
        # END Problem EC
